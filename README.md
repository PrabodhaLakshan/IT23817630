...
⚠️ Important Note
Execution Stability: Due to potential browser synchronization issues or network lag during real-time transliteration, if the test fails during the first run, please run it a second time.


UI Test Scope: The Pos_UI scenario is included for manual UI behavior validation and may not be central to the functional automation script logic, as its primary focus is observing real-time updates.



Use this Code to run the project -  npx playwright test tests/excel.spec.js --project=chromium

Then, auto-open the report.
