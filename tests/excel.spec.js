...
// Remove duplicate test cases with the same `id` (keeps first occurrence)
const uniqueCases = (() => {
  const seen = new Set();
...
test.describe('SwiftTranslator – Excel Driven Tests', () => {
  ...
  // ------------------------------
  for (const tc of uniqueCases.filter(c => /_(Fun)_/.test(c.id))) {
  ...
  // UI Test – Pos_UI_0001
  // ------------------------------
  const uiCase = uniqueCases.find(c => c.id === 'Pos_UI_0001');
